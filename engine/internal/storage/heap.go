...
func (p *HeapPage) Insert(record []byte) (int, error) {
	required := len(record) + slotSize
	if required > p.FreeSpace() {
		return -1, fmt.Errorf("storage: insufficient free space in page %d", p.id)
	}
	...
	writeHeapHeader(p.data, p.hdr)
	return int(p.hdr.SlotCount - 1), nil
}

// Records iterates over stored rows, invoking fn for each entry.
func (p *HeapPage) Records(fn func(record []byte) error) error {
