...
// LastLSN returns the most recent log sequence number generated by the transaction.
func (tx *Transaction) LastLSN() uint64 {
	tx.mu.Lock()
	defer tx.mu.Unlock()
	return tx.lastLSN
}

// SetLastLSN updates the last recorded log sequence number for the transaction.
func (tx *Transaction) SetLastLSN(lsn uint64) {
	tx.mu.Lock()
	tx.lastLSN = lsn
	tx.mu.Unlock()
}
