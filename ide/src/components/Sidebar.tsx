...
export default function Sidebar({ metadata, search, onSearchChange, onSelectTable }: SidebarProps) {
      ...
      <div className="flex-1 overflow-y-auto px-2 py-3 text-sm text-slate-700 dark:text-slate-200">
        ...
        {filtered.map((table) => (
          <div key={table.name} className="mb-3 rounded-md px-2 py-1 hover:bg-slate-100 dark:hover:bg-slate-800">
            <button
              type="button"
              className="w-full text-left font-semibold text-slate-900 dark:text-slate-100"
